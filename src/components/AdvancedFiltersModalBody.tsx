...
export const AdvancedFiltersModalBody: React.FC = ({filtersData, setFiltersData, applyFilterPressed, seenFilter}) => 
    {
    const [filterValues, setFilterValues] = useState([])
    const [showUnseen, toggleSeen] = useState(!seenFilter)
        ...
        let ds = filterValues.map((itm) => {
          if (itm.category === category) {
            itm.items = itm.items.map((el) => {
              ...
              else if(category == 'release_year' || category == 'ratings'){
                el.selected = false
              }
              return  el;
            });
    ...
    const applyFilters = () => {
      setFiltersData(filterValues, false)
      applyFilterPressed(filterValues, showUnseen)
    }
          ...
          >
            <TouchableOpacity activeOpacity={1}>
              {filterValues?.length > 0 ?
                <AdvanceFiltersView filterValues={filterValues} showUnseen={showUnseen} toggleSeen={toggleSwitch} chipClicked={chipClicked}  />
              : null
              }
