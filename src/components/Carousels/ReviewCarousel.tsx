...
import SocialFeedCard from "components/Cards/SocialFeedCard";
import {
  View,
  Dimensions,
  StyleSheet,
...
export type ReviewCarouselProps = {
  ...
  isGuestFlow: boolean;
  showLoginModalCb: Function
};
const ReviewCarousel: FC<ReviewCarouselProps> = ({ data, navigation, isGuestFlow = false, showLoginModalCb }) => {
  const renderCarouselItem = useCallback(({ item }) => {
    return (
      <View style={{ flex: 1 }}>
        <SocialFeedCard showLoginModalCb={showLoginModalCb} item={item} navigation={navigation} home={true} isGuestFlow={isGuestFlow} />
      </View>
    );
  }, []);
  ...
  return (
    <View style={{ marginTop: 0 , marginLeft: isGuestFlow ? 0 : -16}}>
      <Carousel
        ref={(c) => {
          _carousel = c;
        }}
